...
const ABI = [
    "function multiSend(address[] recipients) external",
...
async function main() {
    ...
    if (BigInt(currentSupply) + addycount > BigInt(maxSupply)) {
        throw new Error(`Minting ${addresses.length} would exceed max supply`);
    }

    // Execute multiSend
    console.log(`\nMinting 1 NFT for each of ${addresses.length} addresses...`);
    const tx = await contract.multiSend(addresses);
    ...
    console.log(`Successfully minted ${BigInt(newSupply) - BigInt(currentSupply)} NFTs`);
}
